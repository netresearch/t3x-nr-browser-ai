 ...
 *
 * demo/check-accessibility.mjs covers what is decidable from markup alone. The
 * defects it cannot see — colour contrast above all, because that needs
 * resolved CSS and a compositing model — are what this file is for. This page
 * passes today; the point of the gate is that it keeps passing, because the
 * same audit found 213 failures on a sibling site whose static gate was equally
 * green.
 *
 *   node demo/axe-audit.mjs [output-dir] [base-path]
 *
 ...
 *
 * Not only index.html: a discovery that looks for index.html alone silently
 * skips every page named after its content and still reports success.
 */
...
/**
 * The route shape a page belongs to: a named page collapses to '*.html'. Pages
 * of one shape are the same template with different prose, so auditing all of
 * them costs deploy time and finds what the first one already found.
 */
...
async function main() {
  ...
  if (routes.length === 0) {
    throw new Error(`no pages found in ${OUTPUT} — run demo/build.mjs first`);
  }
  ...
  for (const { route } of sampled) {
    for (const scheme of SCHEMES) {
      ...
      // silent asset error turns this gate into a rubber stamp. Any same-origin
      // request that does not succeed is therefore a failure in its own right.
      const broken = [];
      ...
      if (broken.length) {
        failures.push({
          ...
          violation: {
            ...
            helpUrl: 'https://github.com/netresearch/t3x-nr-browser-ai/blob/main/demo/axe-audit.mjs',
